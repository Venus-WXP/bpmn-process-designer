<template>
  <div class="panel-tab__content">
    <el-table :data="elementListenersList" size="mini" border>
      ...
      <el-table-column label="监听器ID" min-width="80px" prop="id" show-overflow-tooltip />
      ...
      <el-table-column label="操作" width="90px" align="center">
        <template #default="{ row, $index }">
          <el-button type="text" size="mini" @click="openListenerForm(row, $index)">编辑</el-button>
          <el-divider direction="vertical" />
          <el-button type="text" size="mini" style="color: #ff4d4f" @click="removeListener(row, $index)">移除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer :visible.sync="listenerFormModelVisible" title="任务监听器" :size="`${width}px`" append-to-body destroy-on-close>
      <el-form :model="listenerForm" label-width="96px" ref="listenerFormRef" @submit.native.prevent>
        ...
        <el-form-item label="监听器ID" prop="id" :rules="{ required: true, message: '请输入监听器ID', trigger: ['blur', 'change'] }">
        ...
        <template v-if="listenerForm.listenerType === 'scriptListener'">
          <el-form-item
            ...
            :rules="{ required: true, trigger: ['blur', 'change'], message: '请输入脚本格式' }"
          >
          ...
          <el-form-item
            ...
            prop="value"
            key="listener-script"
            :rules="{ required: true, trigger: ['blur', 'change'], message: '请输入脚本内容' }"
          >
          ...
          <el-form-item
            ...
            :rules="{ required: true, trigger: ['blur', 'change'], message: '请输入资源地址' }"
          >
      ...
      <p class="listener-filed__title">
        ...
        <el-button type="primary" icon="el-icon-plus" @click="openListenerFieldForm(null)">添加字段</el-button>
      </p>
      <el-table :data="fieldsListOfListener" size="mini" max-height="240" border fit style="flex: none">
        ...
        <el-table-column label="操作" width="100px" align="center">
          <template #default="{ row, $index }">
            <el-button type="text" size="mini" @click="openListenerFieldForm(row, $index)">编辑</el-button>
            <el-divider direction="vertical" />
            <el-button type="text" size="mini" style="color: #ff4d4f" @click="removeListenerField(row, $index)">移除</el-button>
          </template>
        </el-table-column>
      </el-table>

      <div class="element-drawer__button">
        <el-button icon="el-icon-close" @click="listenerFormModelVisible = false">取 消</el-button>
        <el-button type="success" icon="el-icon-finished" @click="saveListenerConfig">保 存</el-button>
      </div>
    </el-drawer>
    ...
    <el-dialog title="字段配置" :visible.sync="listenerFieldFormModelVisible" width="600px" append-to-body destroy-on-close>
      ...
      <template slot="footer">
        <el-button icon="el-icon-close" @click="listenerFieldFormModelVisible = false">取 消</el-button>
        <el-button type="success" icon="el-icon-finished" @click="saveListenerField">保 存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import { initListenerForm, initListenerType, eventType, listenerType, fieldType } from "./util-self";
...
export default {
  ...
  methods: {
    ...
    openListenerFieldForm(field, index) {
      this.listenerFieldForm = field ? { ...field } : {};

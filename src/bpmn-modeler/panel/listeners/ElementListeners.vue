<template>
  <div class="panel-tab__content">
    <el-table :data="elementListenersList" size="mini" border>
      ...
      <el-table-column label="事件类型" min-width="100px" :formatter="row => (row.event === 'start' ? '开始事件' : '结束事件')" />
      ...
      <el-form :model="listenerForm" label-width="96px" ref="listenerFormRef" @submit.native.prevent>
        <el-form-item label="事件类型" prop="event" :rules="{ required: true, message: '请选择事件类型', trigger: ['blur', 'change'] }">
          <el-select v-model="listenerForm.event">
            <el-option label="开始事件" value="start" />
            <el-option label="结束事件" value="end" />
          </el-select>
        </el-form-item>
        ...
        <template v-if="listenerForm.listenerType === 'scriptListener'">
          <el-form-item
            ...
            :rules="{ required: true, message: '请输入脚本格式', trigger: ['blur', 'change'] }"
          >
...
import { initListenerType, initListenerForm, listenerType, fieldType } from "./util-self";
